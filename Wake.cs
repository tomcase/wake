    ...
    public static class Wake
    {
        public static async Task WakeOnLan(string macAddress)
        {
                ...
                n.NetworkInterfaceType != NetworkInterfaceType.Loopback && n.OperationalStatus == OperationalStatus.Up))
            {
                ...
                foreach (MulticastIPAddressInformation multicastIpAddressInformation in iPInterfaceProperties
                    .MulticastAddresses)
                {
                    IPAddress multicastIpAddress = multicastIpAddressInformation.Address;
                    if (multicastIpAddress.ToString()
                        .StartsWith("ff02::1%",
                            StringComparison.OrdinalIgnoreCase)) // Ipv6: All hosts on LAN (with zone index)
                    {
                        UnicastIPAddressInformation unicastIpAddressInformation = iPInterfaceProperties.UnicastAddresses
                    ...
                    {
                        UnicastIPAddressInformation unicastIpAddressInformation = iPInterfaceProperties.UnicastAddresses.FirstOrDefault(u => u.Address.AddressFamily == AddressFamily.InterNetwork && !iPInterfaceProperties
                            .GetIPv4Properties().IsAutomaticPrivateAddressingActive);
                        if (unicastIpAddressInformation != null)
                        {
                            await SendWakeOnLan(unicastIpAddressInformation.Address, multicastIpAddress, magicPacket);
                            break;
